#!/usr/bin/env python3
# ============================================================
# LMS 图书馆管理系统 · 远端部署驱动器（在【本地 Windows/Linux】执行）
#
...
#   python deploy/remote-deploy-runner.py --host <服务器公网IP> --user ubuntu `
...
def parse_args():
    ap = argparse.ArgumentParser(description="LMS 图书馆管理系统 · 远端部署驱动器")
    ...
    ap.add_argument("--bundle", default="lms-deploy.tar.gz", help="本地部署包路径")
    ap.add_argument("--remote-dir", default="/opt/lms", help="远端部署目录")
...
def main():
    ...
    print()
    if code == 0:
        print(f"\033[32m\033[1m[部署成功] http://{args.host}:8080/（LMS 默认端口；以部署汇总输出为准）\033[0m")
    else:
