...
<section v-if="currentPage === 'analysis'">
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div class="glass-card rounded-2xl p-6">
                                    <CardHeader :title="$t('analysis.popular')" bar-class="from-indigo-500 to-fuchsia-500">
                                        <template #actions>
                                    ...
                                    <ul class="space-y-4">
                                        <li v-if="popularBooks.length===0" class="text-slate-400 text-sm text-center py-6">{{ $t('common.empty') }}</li>
                                        <li v-for="(book, idx) in popularBooks" :key="idx" class="text-sm">
                                            <div class="flex items-center justify-between mb-1.5 gap-3">
                                                ...
                                                <span class="text-slate-500 font-mono text-xs flex-shrink-0">{{ $t('common.unitTimes', { n: book.borrowCount }) }}</span>
                                            </div>
                                ...
                                <div class="glass-card rounded-2xl p-6">
                                    <CardHeader :title="$t('analysis.categories')" bar-class="from-violet-500 to-fuchsia-500">
                                        <template #actions>
                                    ...
                                    <div class="flex flex-col items-center">
                                        ...
                                        <div class="flex flex-wrap justify-center gap-2">
                                            <div v-if="categoryStats.length===0" class="text-slate-400 text-xs py-2">{{ $t('common.empty') }}</div>
                                            <span v-for="(cat, i) in categoryStats" :key="i" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/60 border border-slate-200/70 text-xs text-slate-600"><span class="w-2 h-2 rounded-full" :style="{background: chartColors[i%chartColors.length]}"></span>{{ cat.category }} · {{ $t('common.unitVolumes', { n: cat.count }) }}</span>
                                        </div>
                                    </div>
                                </div>

                                <div class="glass-card rounded-2xl p-6 lg:col-span-2">
                                    <CardHeader :title="$t('analysis.peak')" bar-class="from-sky-400 to-indigo-500">
                                        <template #actions>
                                    ...
                                    <div class="h-52 flex items-end justify-between gap-2 pt-8">
                                        <div v-if="peakTimeStats.length===0" class="w-full text-center text-slate-400 text-sm">{{ $t('common.empty') }}</div>
                                ...
                                <div class="glass-card rounded-2xl p-6">
                                    <CardHeader :title="$t('analysis.stagnant')" bar-class="from-amber-400 to-rose-400">
                                        <template #actions>
                                    ...
                                    <ul class="text-sm">
                                        ...
                                        <li v-if="stagnantBooks.length===0" class="py-6 text-center text-slate-400">{{ $t('common.empty') }}</li>
                                    </ul>
                                </div>
                                <div class="glass-card rounded-2xl p-6">
                                    <CardHeader :title="$t('analysis.active')" bar-class="from-emerald-400 to-teal-500">
                                        <template #actions>
                                    ...
                                    <ul class="text-sm">
                                        <li v-for="(u,i) in activeUsers" :key="i" class="py-2.5 flex justify-between items-center gap-2 border-b border-slate-100 last:border-0">
                                            ...
                                            <span class="text-indigo-600 font-bold font-mono text-xs flex-shrink-0">{{ $t('common.unitTimes', { n: u.borrowCount }) }}</span>
                                        </li>
                                        <li v-if="activeUsers.length===0" class="py-6 text-center text-slate-400">{{ $t('common.empty') }}</li>
                                    </ul>
                                </div>
                            </div>
                        </section>
