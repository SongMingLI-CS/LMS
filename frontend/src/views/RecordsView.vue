                            ...
                            <div class="glass-card rounded-2xl overflow-hidden">
                                ...
                                <table class="w-full text-left text-sm">
                                    ...
                                    <tr v-for="r in filteredRecords" :key="r.id" class="hover:bg-indigo-50/40 transition-colors">
                                        ...
                                        <td class="px-6 py-3.5"><span :class="['px-2.5 py-1 rounded-full text-xs font-semibold', r.status==='borrowed' ? (isOverdue(r.dueDate) ? 'bg-rose-50 text-rose-500' : 'bg-indigo-50 text-indigo-600') : r.status==='returned' ? 'bg-emerald-50 text-emerald-600' : r.status==='reserved' ? 'bg-amber-50 text-amber-600' : r.status==='awaiting_pickup' ? 'bg-violet-50 text-violet-600' : 'bg-slate-100 text-slate-600']">{{ $t(statusLabelKey(r.status)) }}</span></td>
...
import { statusLabelKey } from '../utils/helpers'
export default {
  name: 'RecordsView',
  setup() {
    const s = useLms()
    return { ...bindStore(s), statusLabelKey }
  }
}
</script>
